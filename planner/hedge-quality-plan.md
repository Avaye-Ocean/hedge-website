...
## Round 72 — End-to-end BUILD/VERIFY of the vendor product create + edit flow (image upload focus; admin + mobile)

**Type:** END-TO-END TRACE/VERIFY + FIX. Scope: hedge-wears-admin + hedge-mobile-app (`develop-extended`). Every hop traced from the form submit to the live `products` contract — not a param-level read. hedge-web-app confirmed customer-only (no product-management surface). Product surfaces already closed in prior rounds (delivery/orders/payments/auth/address/reviews/wishlist) were not re-audited.

### Backend contract (READ-ONLY, `vendorstack-backend/src`)
- `POST /products` (create) + `PATCH /products/:id` (edit) — `CreateProductDto` / `UpdateProductDto` (PartialType + required `businessId`,`userId`). Body is JSON; controller wraps `FileInterceptor('videoFile')` so the **only** multipart file it reads is `videoFile`.
- **IMAGE FINDING (the make-or-break shape).** A product image is a **base64 `data:image…` data-URL string in the `photo` field** (single, required on create) plus `thumbnailPhotos: string[]`. The service Cloudinary-uploads it server-side (`uploadPhotosWithPhotoLinks([photo], PRODUCT_PHOTOS)`); `addDocsWithLinks` **passes through** any existing `http` URL unchanged and only uploads `data:` strings — so edit preserves the current photo when unmodified. There is **no** multipart-`File` product-image field and **no** client-side Cloudinary upload. This is exactly the R63 class.
- **Prices are COIN.** DTO carries `sellingPrice`/`costPrice` (string, ≥300) only; the custom `ValidatePipe` runs `injectPaymentFieldsCoinNaira` **after** validation with a default `WalletType.COIN` (no `walletType` sent) → the incoming value is treated as coin and `*Coin`/`*Naira` are injected. Client must send the **raw coin** value. `discountAmount` same. `*Coin` fields are not in the DTO → stripped by `whitelist:true` (no `forbidNonWhitelisted`, so extras strip, they don't 400).
- **Variants:** `ProductVariantDto` = `{ name, display, values: ProductVariantValueDto[] }`; `ProductVariantValueDto` = `{ name (req), price:number (req, ≥0) }`. `price` sent in coin.
- Required on create: `name`, `photo`, `quantity`(≥1), `costPrice`(≥300), `sellingPrice`(≥300), `businessId`, `userId`; `costPrice` < `sellingPrice`. Product `categories`/`genders` are **derived from the business** server-side — the DTO has no such field, so client selections are stripped no-ops.

### Per-surface verdict
- **Admin create** (`products/add/_add-product-view.tsx`) — **bugs fixed (`fc608c4`).** Image shape was already CORRECT (base64 `photo` = `photos[0]`, `thumbnailPhotos` = all base64). **Break 1 (coin/naira):** `sellingPrice`/`costPrice` were `coinToNGN(…)` (naira) with no `walletType` → backend (COIN) double-converted (≈rate²-inflated coin AND naira). **Break 2 (variants):** values sent `{ name, priceCoin, price: coinToNGN(priceCoin) }` → `price` double-converted. Now sends raw coin `sellingPrice`/`costPrice` and variant `{ name, price:<coin> }`; dropped stripped `sellingPriceCoin`/`compareAtPrice*`.
- **Admin edit** (`products/[id]/_product-detail-view.tsx`) — **bugs fixed (`fc608c4`). THE R63-class break:** a changed photo was posted as a **multipart `photos` File** (`FormData` + `Content-Type: multipart/form-data`) — the backend PATCH ignores every multipart field except `videoFile`, so **the new photo was silently dropped, never saved.** Now converts the file with `fileToDataUrl` and sends `photo:<base64>` in the JSON body (matching create). Also fixed the same coin/naira double-convert, and variant values that sent only `priceCoin` with the **required `price` missing → would 400 the whole array** (now `{ name, price:<coin> }`).
- **Mobile create** (`components/manage-store/products/index.tsx`) — **bug fixed (`75754e8`).** Image shape CORRECT (`ProductMedia` yields `data:image…` for `photo`/`thumbNails`), variants CORRECT (`{ name, price:Number(v.price) }` coin). **Break:** `costPrice`/`sellingPrice`/`discountAmount` were `coinToNGN(…)` → COIN-default double-convert. Now raw coin; dropped stripped `*Coin` fields.
- **Mobile edit** (`components/manage-store/EditProduct.tsx`) — **bug fixed (`75754e8`).** Image CORRECT (reuses `ProductMedia`; unchanged photo = existing `http` URL, backend passes through), variants CORRECT. Same coin/naira double-convert fixed.
- **hedge-web-app** — **clean (customer-only).** No product create/edit surface; `product-actions.tsx` is add-to-cart. Nothing to change.

### Backend gaps logged (not changed — READ-ONLY)
1. **Variant price double-conversion (latent).** `injectPaymentFieldsCoinNaira` recurses into nested `variants[].values[].price` (in `MONETARY_FIELDS`) AND the service `variantsBuild` converts coin→naira **again** on the same value → a submitted variant price is rate²-inflated in both `price` and `priceCoin`. Never exercised by vent (its CreateProduct sends no `variants`), so it's a dormant backend bug. The hedge clients now send the correct DTO shape (`price` in coin); a proper fix is backend — `variantsBuild` should not re-convert what the pipe already converted (or the pipe should skip nested variant `price`).
2. **Product-level categories/genders not accepted on create/edit** — derived from the business; DTO has no field, so client selections are stripped. If per-product categories/genders are ever required it's a backend DTO+service change.
3. `main.ts` `ValidationPipe` still has `forbidNonWhitelisted` commented out (only `whitelist:true`) — extras strip rather than 400; why the stray `sellingPriceCoin`/`compareAtPrice`/`status`/`category` fields were harmless pre-fix. (Pre-existing security-rule deviation.)

### Validation
- `npx tsc --noEmit` → **0** in hedge-wears-admin and hedge-mobile-app. Admin husky lint passed (1 pre-existing unrelated warning); mobile husky tsc passed. Mobile stale Expo jest suite skipped per runner policy. Additive; no new deps; no cosmetic churn.

**STATUS: Hedge R72 COMPLETE — product create AND edit traced end-to-end (admin + mobile) against the live products contract. Image shape: client must send base64 `data:image` in `photo` (+ `thumbnailPhotos[]`), only `videoFile` is multipart. Found + fixed the R63-class break — admin edit posted the photo as a multipart `photos` File that the backend drops (photo silently never saved); now base64 JSON. Also fixed a coin/naira double-conversion across all four create/edit surfaces (naira sent under a COIN-default wallet) and malformed variant values (missing required `price` on edit → 400; double-converted on create). Create/edit bodies now match the DTO exactly; images verified base64-in-JSON with http-URL passthrough for edit preservation. Backend variant-price double-conversion logged as a dormant gap. tsc 0 both touched repos (`fc608c4`, `75754e8`).**

---

## Round 73 — End-to-end BUILD/VERIFY of the notifications + settings/account-preferences flows (web + mobile)

**Type:** END-TO-END TRACE/VERIFY + FIX. Scope: hedge-web-app + hedge-mobile-app (`develop-extended`). Each hop traced from the client request to the live backend contract in `vendorstack-backend/src`. hedge-wears-admin has no customer notifications/settings surfaces (out of scope). Prior-closed surfaces (delivery/orders/payments/auth/address/reviews/wishlist/product-create-edit) not re-audited.

### Backend contract (READ-ONLY, `vendorstack-backend/src`)
- **Notifications list:** `GET /users/notifications` (`users.controller.ts:951`). The controller **force-overrides** `req.query.notificationByOwnerIds = req.user._id` — the list is **always** owner/recipient-scoped by the session; the client must not (and cannot usefully) set ownership itself. Response is `getPaginated(...)` → `{ results, totalRecords, totalPages, currentPage, nextPage, previousPage, perPageLimit }`. Each record (from `NotificationSchema.getNotifications`): `{ _id, owner, accounts[], entity, entityType (BehaviorEntityType: USER/BUSINESS/PRODUCT/POST/AD/COMMENT/THEME/ORDER/SUBSCRIPTION/REVIEW/SYSTEM), entityMessage, message, type (BehaviorType: FOLLOW/LIKE/…), counts, isNotify, createdAt }`. **Read-state field is `isNotify`** (true = read/notified); there is no `isRead` on the wire (clients fall back `!isNotify`).
- **Query-filter trap:** `notificationByAccountIds` maps to `{ accounts: {$in} }` (`query.util.ts:272`) — i.e. the **actors** on a notification (who followed/liked), **not** the recipient. Combined with the forced owner filter it yields notifications where you are both owner AND actor ≈ empty. `notificationByOwnerIds` maps to `{ owner }`.
- **Mark read:** `PATCH /users/notifications/read-all` (`markAllNotificationsRead` → `updateMany({owner:userId,isNotify:false},{isNotify:true})`) and `PATCH /users/notifications/:notificationId/read` (`markNotificationRead` → `updateOne({_id, owner:userId},…)`). Both **session-scoped by `req.user._id`**; no body.
- **Push token:** registered via the user login/create/update DTOs — `deviceToken` (native FCM, mobile) and `webToken` (browser FCM, web). Backend pushes to **both** fields, so a web token written into `deviceToken` silences the same user's mobile push (the R62 class).
- **Settings:** `GET /users/me/settings` (+ `/users/:userId/settings`) and `PATCH` same → `UserUpdateSettingsDto` sections `notifications` / `privacy` / `content` / `security` / `account`. `updateSettings` builds **dot-path `$set`** (`settings.<section>.<key>`) so it is siblings-safe even on partials, and returns the **full resolved settings** (`resolveSettings` merges defaults). Fields are social-creator-centric (newFollower/newLike/newGift/newSubscriber/profileVisibility/allowDirectMessages/twoFactorEnabled…); `whitelist` strips unknown keys.

### Per-flow verdict

**FLOW 1 — Notifications**
- **hedge-web** (`components/shared/notifications-sheet.tsx`, `api/user/index.tsx`) — EXISTS (bell + slide-in sheet + unread badge + date grouping + skeleton/empty states). **2 bugs fixed + 1 gap completed (`bf83bdd`):**
  - **BUG (empty-list):** `getNotifications(userId)` sent `?notificationByAccountIds=${userId}` → filtered by actor, so the list was almost always empty. Now calls `GET /users/notifications` plain; backend owner-scopes by session.
  - **BUG (icons):** `getTypeConfig(item.type)` used `type` (BehaviorType FOLLOW/LIKE) against a `TYPE_CONFIG` map keyed by **entityType** (USER/PRODUCT/ORDER/…) → **every** icon fell to the default Bell. Now `getTypeConfig(item.entityType)`.
  - **GAP:** the "Mark all as read" button was commented out (unread badge had no way to clear). Wired `useMarkAllNotificationsRead()` → `PATCH /users/notifications/read-all` (+ `useMarkNotificationRead` per-item hook), optimistic-free but invalidates the notifications query; button shown only when `unreadCount>0`.
  - Push: `FcmInitializer` writes the browser FCM token to **`webToken`** (`api/user/index.tsx:126`) — **R62 fix intact, not regressed.**
- **hedge-mobile** (`app/notifications.tsx`, `components/notifications/`, `hooks/apihooks/user.ts`) — EXISTS (FlashList + infinite scroll + skeleton/empty/error + deep-link on tap; `NotificationItemCard`). **1 bug fixed + 1 gap completed (`d5946df`):**
  - **BUG (empty-list):** screen passed `notificationByAccountIds:[user._id]` (also the hook's `enabled` gate + queryKey) → same actor-filter empty-list bug. `useGetNotifications` now reads user from `useUserStore`, gates `enabled` on `!!user._id`, keys the query on `user._id`, and strips any `notificationByAccountIds` before the call; screen calls `useGetNotifications()`.
  - **GAP:** the "Clear notifications" header was dead non-interactive `Text`. Relabelled to "Mark all read" and wired `useMarkAllNotificationsRead()` → `PATCH /users/notifications/read-all`, shown only when an unread item exists.
  - Icon map (`NotificationItemCard.getIcon(item.entityType)`) already keyed on entityType — `ORDER`/`SYSTEM` match; other entity types fall back to the default Shop icon (acceptable, not a bug).
  - Push: `deviceToken` (native FCM) registered at signup/login (`app/(auth)/…`) — correct field for the native app.

**FLOW 2 — Settings / account preferences**
- **hedge-web** — **NOT present (valid).** No backend-synced account-preferences screen. `account/page.tsx` is profile/orders/addresses; `coin/settings` is wallet config. The backend `GET/PATCH /users/me/settings` exists but is unused (social-creator-centric, out of storefront scope). Logged; not fabricated.
- **hedge-mobile** (`app/settings.tsx` → `components/settings/`) — EXISTS but is **device/local-only by design (valid).** Manages OS notification permission (`messaging().requestPermission()`), dark mode (nativewind), language, currency, and account deactivation (`updateAccountStatus → INACTIVE`). It intentionally does **not** call the backend settings endpoints — those sections are social-platform prefs not relevant to Hedge customers. Confirmed, not wired to backend; not a bug.

### Backend gaps logged (not changed — READ-ONLY)
1. **`notificationByAccountIds` is a foot-gun for recipient lists** — its name reads like "the user's notifications" but it filters by *actors*. Both hedge clients (and any future one) naturally reach for it and get an empty list. Backend could ignore/deprecate the param on the recipient endpoint (owner is already forced) or rename for clarity.
2. **No wire `isRead`** — read-state is `isNotify` (dual-purpose: also set true by the 20-min `notifyMessage` push cron). So "unread" only reflects notifications younger than ~20 min; older ones read as read regardless of user action. A dedicated `readAt`/`isRead` distinct from the push-dedupe flag would let clients show true unread state.
3. **Settings surface unused by storefront** — the `UserUpdateSettingsDto` sections are creator/social-centric; no storefront-relevant subset (e.g. order/payment push prefs) is separable without a backend shape that Hedge could adopt. Noted as product gap, not a bug.

### Validation
- `npx tsc --noEmit` → **0** in hedge-web-app and hedge-mobile-app. Web husky lint+build passed on push; mobile husky tsc passed on commit+push. Mobile stale Expo jest suite skipped per runner policy. Additive; reused existing hooks/patterns; no new deps; no cosmetic churn.

**STATUS: Hedge R73 COMPLETE — notifications traced end-to-end (web + mobile) against the live `GET/PATCH /users/notifications*` contract; settings confirmed (web: not present; mobile: device-local by design — both valid, not fabricated). Found + fixed the same actor-filter empty-list bug on BOTH clients (`notificationByAccountIds` filters by actor, backend already owner-scopes by session), a web-only icon bug (keyed on `type` not `entityType` → all Bell), and completed mark-all-read on both (web button was commented out, mobile "Clear" was a dead label) → `PATCH /users/notifications/read-all`. R62 web `webToken` fix verified intact. Backend `notificationByAccountIds` naming + `isNotify`-as-read-state logged as gaps. tsc 0 both touched repos (`bf83bdd`, `d5946df`).**

## Round 74 — End-to-end BUILD/VERIFY of the cart→checkout MONETARY sub-flows: voucher/coupon, points, convenience fee (web + mobile)

Traced each hop against the live backend contract (`orders.service.ts`, `rewards.service.ts`, `validate.pipe.ts` + `currency.util.injectPaymentFieldsCoinNaira`). Preflight clean on all three repos.

### Backend contract (verified, READ-ONLY)
- **Voucher/coupon:** applied via a **separate redeem call** — `POST /rewards/vouchers/:code?vendorIds=…` → credits the user's wallet points (`$inc wallet.point`, `wallet.pointCoin`) and returns the reward doc `{ point, pointCoin, voucherCode, status, allowUsageOnUserIdPage, … }`. **There is no `voucherCode` field on the order DTO and no `discountAmount*` on the redeem response.** The order is NOT discounted by the redeem itself.
- **Points:** `BulkCreateOrderDto.point` (single field). The global amount-injection pipe (`validate.pipe.ts` → `injectPaymentFieldsCoinNaira`, default `walletType=COIN`) treats a COIN request's `point` as coin, derives `pointCoin`, and rewrites `point` to its naira equivalent. `orders.service` subtracts `point`/`pointCoin` from `transactionTotalAmount(Coin)` → reduces the authoritative `totalPayableAmount(Coin)`. Server-authoritative; caps enforced (`point > order amount` and `totalPayableAmount <= point` both throw — the latter caps an applied credit at just under half the order).
- **Convenience fee:** wallet order → server `getOrderCommission(coinTotal)` = `ratio 0.005, min 1, cap 6` coins, added at charge time. Non-paystack single-step flow; **no pre-charge quote endpoint** (`cartCheckout` just calls `createOrder`). `createOrder` returns the full transaction (`getTransactionById`) with authoritative `totalPayableAmountCoin`, `convenienceFeeCoin`, `pointAmountCoin`.
- Note: `main.ts` uses `ValidatePipe({ whitelist: true })` with `forbidNonWhitelisted` **commented out** → unknown body fields are stripped (not 400).

### Sub-flow verdicts
- **VOUCHER — present on BOTH; BUGS FIXED.** Both clients had a voucher input that did nothing useful and mis-fed the order:
  - Both read `discountAmountCoin ?? discountAmount ?? 0` off the redeem response — fields that don't exist → discount always **0** (voucher visibly did nothing to the total).
  - **web** additionally did `setPromotion(reward._id)` → sent the reward id as the order `promotionId` (bogus promotion linkage on the transaction).
  - **mobile** additionally sent `{ voucherCode }` on `POST /orders` — not a DTO field → silently stripped → the redeemed points were never applied; order charged full price.
  - **Fix (both):** read `reward.pointCoin`; drop the bogus `promotionId`/`voucherCode`; apply the credited coins via the order-create **`point`** field (server subtracts from `totalPayableAmountCoin`); cap applied coins at `floor((subtotal + delivery) / 2) - 1` to respect the backend `totalPayableAmount <= point` guard; grand total now mirrors `(subtotal + delivery − voucher)`. Web refreshes the profile after redeem. Hashes: web `781d866`, mobile `53eed67`.
- **POINTS — present (via the voucher path).** Neither app exposes a standalone "apply earned points" input (vent-web has one; noted as a parity opportunity, not fabricated here). The voucher-credited points are now applied to the order as `point` (coin), server-authoritative. Request shape verified: `POST /orders { orderDetails, customerId, addressId, isWalletPayment:true, sourceId, point } (+ businessId/userId)`.
- **CONVENIENCE FEE — present server-side, correctly NOT client-invented (valid gap).** For a wallet order the ≤6-coin order commission is computed at charge time and is not previewed (no quote endpoint). Displayed grand total can be up to 6 coins below the final `totalPayableAmountCoin`; the server total is authoritative. Documented in both developer-guides; deliberately not re-derived client-side.

### Backend gaps logged (not changed — READ-ONLY)
1. **No pre-charge quote endpoint for wallet orders** — `cartCheckout` creates+charges; `createOrder` with `isWalletPayment:false` previews but persists an orphan pending order/transaction. A non-persisting quote returning `totalPayableAmountCoin`/`convenienceFeeCoin` would let clients show the exact charge (incl. the ≤6-coin commission) before placing the order.
2. **`totalPayableAmount <= point` guard caps vouchers at ~half the order** — because `point` is subtracted once, a voucher covering more than ~50% of the payable is rejected. Surprising for "full-value" vouchers; clients must cap client-side. Consider allowing points up to (but not equal to) the full payable.
3. **`forbidNonWhitelisted` disabled** (`main.ts`) — unknown order fields (e.g. the old `voucherCode`) are silently stripped rather than rejected, which masked the mobile bug. Security rules require it on.
...
## Round 77 — Profile/account edit + wallet/coin/transaction display (web + mobile)

End-to-end BUILD/VERIFY of two un-traced customer surfaces against the live backend: (1) profile/account edit — name, photo, password change; (2) wallet/coin/transaction display consistency.

### Backend contracts verified (READ-ONLY)
- `PUT users/:userId` (`updateUser`, `UserUpdateDto`) — ownership via `req.user._id` (session, param ignored). Accepts `firstName`/`lastName`/`photo`/`phone`/`username`/`userHandle`/`socialMediaHandles`/etc. **`photo` must be a `data:image` data URL** — backend uploads it to Cloudinary itself (throws `"Photo must include a data image"` otherwise). `phone` is dropped if `phoneVerification`; `username` dropped for non-admins / verified email. Backend `ValidatePipe` is `whitelist: true` (strips unknown body keys) but **NOT** `forbidNonWhitelisted` — so stray body fields (`userId`/`businessId`/`sourceId`/`email`) are silently stripped, not rejected.
- `PUT users/:userId/profile-photo` (`UserAddPhotoDto {photo, userId}`) — base64 data URL only, session-scoped ownership. (Web uses this route; mobile uses `updateUser` — both base64.)
- `PUT users/:userId/change-password` (`UserChangePasswordDto {oldPassword, newPassword, confirmPassword}`) — session ownership; response `.select('-password')` (no hash leak).
- Wallet fields on `user.wallet`: `currentBalance`/`currentBalanceCoin`, `bookBalance*`, `pendingBalance*`. Transaction amounts: `totalPayableAmountCoin` → `totalAmountCoin` → `amountCoin`, plus `type`/`status`/`createdAt`.

### FLOW 1 — Profile / account edit
- **Web — CLEAN-with-evidence.** `edit-profile` dialog sends `{firstName, lastName}` via `useEditProfile` (PUT `users/:userId`) and invalidates `USER_PROFILE` on success (store re-sync). Photo picked in-dialog + the standalone `profile-photo-upload` both go through `fileToDataUrl` → base64 → the dedicated `PUT users/:userId/profile-photo` `{photo, userId}` (R63 base64 fix intact) and invalidate `USER_PROFILE`. `change-password` sends `{oldPassword, newPassword, confirmPassword}` — matches DTO exactly; response only toasts, no token/password leak.
- **Mobile — BUG FIXED (`89b5fde`).** `personal-details` seeded `state.photo` with the existing **Cloudinary URL** and re-sent it on every save; the backend rejects any non-`data:image` photo, so **every name-only edit failed for any user who already had a profile photo** (`"Photo must include a data image"`). Fix: `delete data.photo` unless it `startsWith('data:image')` (only send a freshly picked image); newly picked images default mime to `image/jpeg` when the picker omits it. Photo on mobile flows through `updateUser` as base64 (backend → Cloudinary), not the dedicated route — both platforms base64, no multipart. Store re-syncs via `useGetUserInfo().refetch()` → `saveUser`. Change-password screen sends `{oldPassword, newPassword, confirmPassword, userId}` (userId stripped by whitelist) — clean, session ownership, no leak.

### FLOW 2 — Wallet / coin / transaction display
- **Web — CLEAN-with-evidence.** Balances read `wallet.currentBalanceCoin` coin-first with a `coinToFiat` naira equivalent (`hedgecoin-balance`, buy/preset views). `transaction-item` renders coin fields (`totalPayableAmountCoin` → `totalAmountCoin` → `amountCoin`) via `CoinText` (coin-first) + fiat equivalent, with `TYPE_LABEL` map, debit/credit sign, status colour, `PPP` date. `transactions-list` has error+retry, skeleton, and empty-state; list paginates via `useGetTransactions` (limit 20, `transactionByUserId`). No coin/naira mislabel.
- **Mobile — CLEAN-with-evidence.** `BalanceCard` shows a **coin** card (`currentBalanceCoin`, label "HedgeCoin Balance", + fiat equiv) and a **naira** card (`currentBalance`, currency-labelled) — correct coin/naira split. `TransactionItem` uses the identical coin-field priority + fiat equivalent, `TYPE_LABELS`, debit/credit sign, status colour, date — matches web. Wallet screen reads live `user.wallet` from the store; transactions refetch on focus; error + empty states present. Balance re-sync after money actions confirmed: `ReviewBuycoin` refetches `getUserInfo` (→ `saveUser`) + invalidates `WALLET` on Paystack/crypto success.

### Backend gaps logged (not changed — READ-ONLY)
1. **`forbidNonWhitelisted` is commented out** in `main.ts` (`whitelist: true` only). Not a hedge defect (extra fields are stripped, not rejected) but noted — a typo'd field name would silently no-op rather than error.
2. **Mobile email field is a no-op** — `personal-details` exposes an editable Email input, but the DTO field is `username` (not `email`) and is gated by `emailVerification`; the sent `email` key is stripped. Left as-is (out of the name/photo/password scope; email change needs a verify flow, not built).

### Validation
- `npx tsc --noEmit` → **0** in hedge-mobile-app (touched). hedge-web-app **not touched** (clean-with-evidence, no commit). Additive single-file fix; reused existing hooks/patterns; no new deps; no cosmetic churn.

**STATUS: Hedge R77 COMPLETE — profile/account edit + wallet/coin/transaction display traced end-to-end (web + mobile) against the live backend. FLOW 1 PROFILE: web CLEAN-with-evidence (base64 photo on dedicated route, DTO-exact change-password, USER_PROFILE re-sync); mobile BUG FIXED (`89b5fde`) — stale Cloudinary URL was re-sent as `photo` and rejected by the backend, breaking every name-only edit for users with a photo; now only a freshly picked base64 image is sent. Profile photo confirmed base64 on BOTH platforms (web dedicated route, mobile via updateUser), no multipart. FLOW 2 WALLET: both platforms CLEAN-with-evidence — coin-first balances (`currentBalanceCoin`) + fiat equivalent, transaction amounts use `totalPayableAmountCoin`/`totalAmountCoin`/`amountCoin` (coin-first, no coin/naira mislabel), type labels/status/date/empty+error states present, balance re-sync after buy-coin verified. Backend gaps logged: forbidNonWhitelisted disabled, mobile email field is a stripped no-op. tsc 0 mobile (only touched repo).**

## Round 78 — Owner business analytics / metrics (mobile manage-store + admin dashboard)

END-TO-END BUILD/VERIFY on the owner-facing analytics surfaces — the least-traced remaining hedge area. Focus: metric ACCURACY — the time-bucket param class, coin-vs-naira KPI/chart labeling (the class R62 fixed on the admin revenue tile — checked for MORE instances), correct revenue field, and unit-consistent AOV. Traced against the live backend metrics contracts (READ-ONLY).

### Backend metrics contracts verified (READ-ONLY)
- `getMetricsGroupId` (metrics.util.ts) throws `BadRequestException` unless `byValue`+`byValueType` (HOUR/DAY/DAY_OF_WEEK/WEEK/MONTH) **or** `metricDateRange` is supplied — so a **bare `/metrics/counter` with no time bucket 400s**. Every hedge counter call passes `metricDateRange`; none are bare.
- `sortMetrics` → time-bucketed series (array, each entry `{id, type, ...}`). `sortMetricsCount` → single summed object when no `groupBy`.
- COIN fields: orders `byValueCoin` (=`vendorAmountCoin`, delivered vendor revenue), `byValueCostCoin` (cost of goods), products `byValueSellingPriceCoin`, transactions `byValueAmountCoin`, ads `byValueAmountCoin`. COUNT/other: `byCount`, `totalViews`, `sumRating`. Rewards findReport returns **points-based series** (`byValue`=Σ`point`, `byCount`) — **no counter branch, no coin field**.
- Orders counter (DELIVERED) returns BOTH `byValueCoin` and `byCount` — so delivered revenue and delivered order count come from the same call.

### Mobile — manage-store analytics — ALL CLEAN-with-evidence
- `index.tsx` / `performance.tsx` / `insight.tsx` — order KPIs via `useGetOrderMetricsCounter` with `orderStatus` + `metricDateRange` (never bare); all values are counts (`data.byCount`). No revenue/AOV tile; no coin/naira mislabel.
- `financial-report.tsx` — reads the **transactions list** (`useGetTransactions`, `transactionDateRange` re-queries on date change), Total In/Out + rows use `amountCoin`/`totalAmountCoin` coin-first with the `Logo` coin glyph. No throwing counter.
- `inventory.tsx` — products list; per-unit price coin-first (`sellingPriceCoin` + Logo). Loading/error/empty states present on every screen.

### Admin — dashboard + analytics
- **Home (`_home-view.tsx`) — CLEAN-with-evidence.** Total Revenue = `revenueCounter.byValueCoin` on the **DELIVERED** counter (R62 fix intact — delivered vendor revenue, not cost-of-goods, not all-status), coin-first `CoinIcon` + fiat equiv. Orders=`byCount`; Products/Customers from `totalRecords`. Child widgets all clean: `revenue-overview` (line chart of `byValueAmountCoin` from tx metrics series), `sales-by-category` (soldCount %), `top-selling-products` + `recent-orders` (coin-first `sellingPriceCoin`/`totalAmountCoin` + fiat), `low-stock`/`pending-returns` (no money).
- **Analytics (`_analytics-view.tsx`) — 2 BUGS FIXED (`ad3d467`).**
  1. **Avg Order Value unit-inconsistency** — numerator was DELIVERED revenue (`revenueCounter.byValueCoin`) but denominator was the **all-status** order count (`orderCounter.byCount`, includes pending/canceled/rejected/returned), understating AOV. Fixed to divide by `revenueCounter.byCount` (DELIVERED order count — same order set as the numerator).
  2. **Total Discount Given always 0** — summed non-existent `amountCoin`/`amount` on voucher (Reward) docs. Rewards store the discount in `pointCoin`/`point`; fixed to sum USED vouchers' `pointCoin` (the same coin field the Vouchers page renders as "Coin Amount"). The primary `rewardsMetrics?.byValueCoin` read is inert (rewards endpoint returns a points series, no coin field) so it correctly falls through to the voucher-list sum.
  - Other analytics tiles CLEAN: Total Revenue (`byValueCoin`, DELIVERED, coin-first), Inventory Value (`byValueSellingPriceCoin`, coin-first), Product Views (`totalViews`), Reviews (`byCount`), Avg Rating (`sumRating/byCount`). Charts read `byValueAmountCoin` (coin) / `byCount` from series with `metricDateRange`. Date-range selector re-queries via `useMemo` dep.
- **transactions view — CLEAN-with-evidence.** "Total Revenue" = `totalCounter.byValueAmountCoin` (coin) coin-first; pending/failed = `byCount`; wallet balance coin-first — all counters carry `metricDateRange`. (Semantic note: this tile is total transaction volume, correctly coin-labelled — not the order-revenue class.)
- **content view — CLEAN.** Ad views = `adCounter.totalViews` (count) via ad metrics counter with `metricDateRange`.

### Coin/naira labeling audit result
Every coin KPI/row on both apps is shown coin-first (HGC glyph / `CoinIcon` / `Logo`) with a `coinToFiat` secondary. No coin value is formatted/labelled as naira, and no naira/count value carries a coin icon. Revenue reads the delivered coin field (`byValueCoin`/`vendorAmountCoin`) everywhere. The only defects were the AOV denominator and the wrong voucher discount field — both fixed.

### Backend gaps logged (not changed — READ-ONLY)
1. `GET /rewards/metrics` has **no counter branch and no coin aggregation** — returns a points-based time series only (`byValue`=Σ`point`, `byCount`). Admin voucher tiles therefore derive from the vouchers list (capped at 50), so "Vouchers Redeemed" / discount total can undercount when a store has >50 vouchers. Enhancement, not a hedge defect.
2. Order/product/transaction metrics counters require a time bucket (by design); dashboards correctly use `totalRecords` off list endpoints for plain totals (products/customers).
